...
### Нюансы использования Vagrant через WSL
* Vagrant должен быть установлен на Windows и внутри WSL, версии должны быть одинаковые
* Добиться выполнения команды `vagrant up` из WSL не удалось. Виртуальная машина очень долго грузится. Работает следующая последовательность действий:
* * Закомментировать блоки provision в файле Vagrantfile
* * Запустить `vagrant up` в Windows, дождаться завершения поднятие ВМ

Перед установкой molecule
